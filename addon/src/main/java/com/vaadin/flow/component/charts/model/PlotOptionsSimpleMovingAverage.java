...
/**
 * Simple moving average indicator (SMA). This series requires `linkedTo` option
 * to be set.
 * 
 * Configuration options for the series are given in three levels: 1. Options
 * for all series in a chart are defined in the
 * [plotOptions.series](plotOptions.series) object. 2. Options for all `sma`
 * series are defined in [plotOptions.sma](plotOptions.sma). 3. Options for one
 * single series are given in [the series instance array](series.sma).
 * 
 * <pre>
...
public class PlotOptionsSimpleMovingAverage extends AbstractPlotOptions {
	...
	public PlotOptionsSimpleMovingAverage() {
	}
