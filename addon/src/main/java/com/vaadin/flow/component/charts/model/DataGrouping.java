...
public class DataGrouping extends AbstractConfigurationObject {
	 ...
	 * range and the current data grouping. The default formats are:
	 * 
	 * <pre>
	 * {
	 * 	    millisecond: [
	...
	/**
	 * By default only points within the visible range are grouped. Enabling
	 * this option will force data grouping to calculate all grouped points for
	 * a given dataset. That option prevents for example a column series from
	 * calculating a grouped point partially. The effect is similar to
	 * [Series.getExtremesFromAll](#plotOptions.series.getExtremesFromAll) but
	 * does not affect yAxis extremes.
	 */
